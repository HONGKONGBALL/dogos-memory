...
`packages/native_agent` 的动作策略目前是本地安全模块，尚未成为原厂工具调用的不可旁路
hook。

## 仓库结构

```text
packages/          可复用的 Python 包
  dogos_memory/    SQLite 记忆存储、模型和 CLI
  dogos_demo/      双狗身份门控、配对调度和模拟闭环
  native_agent/    身份包、设备管理、策略、DogOS 适配器和 MCP 端点
apps/              可直接运行的本地应用
  companion/       确定性陪伴状态机原型
  demo_choreo/     固定动作演示与离线回退路径
integrations/      部署到外部运行时的最小桥接代码
  robot_side/      Vbot 侧只读身份桥
scripts/           唯一的本地命令入口、配对和受信设备管理脚本
tests/             DogOS 核心测试；各模块测试与模块同目录
examples/          合成配置样例，不含真实设备资料
docs/              架构、开发和模块边界说明
```
...
uv sync --locked
uv run pytest
./scripts/run-python.sh -m dogos_demo encounter \
...
PYTHONDONTWRITEBYTECODE=1 uv run pytest packages/native_agent/tests apps/companion/tests apps/demo_choreo/tests -q
```
...
目录约定和迁移规则见 [`docs/REPOSITORY_LAYOUT.md`](docs/REPOSITORY_LAYOUT.md)，更多 Agent
实现说明见 [`packages/native_agent/README.md`](packages/native_agent/README.md)。
